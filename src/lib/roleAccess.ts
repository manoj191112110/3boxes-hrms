...
export function isModuleEnabledForTenant(tenantSlug: string, moduleKey: string): boolean {
  ...
  // An empty Set must not hide every module — that happens when the
  // tenant-modules API returns no rows, and it makes /modules look blank
  // after the first visit.
  if (dbOverride && dbOverride.size > 0) return dbOverride.has(moduleKey);
