...
/** Dedupe in-flight fetches so Sidebar + Modules don't fire 4 parallel calls. */
...
export const useTenantModuleStore = create<TenantModuleState>((set, get) => ({
  ...
  fetchModules: async (tenantId: string, tenantSlug: string) => {
    ...
    if (get().fetched[tenantSlug]) return;
    const existing = inflightBySlug.get(tenantSlug);
    ...
    const run = (async () => {
      ...
      try {
        ...
        const modules: { enabled: boolean; key: string }[] = data.modules || [];
        // Empty payload is a failed/partial response — keep whitelist fallback
        // so the Modules tab does not go blank after the first visit.
        if (modules.length === 0) {
          set({ loading: false });
          return;
        }
        ...
        inflightBySlug.delete(tenantSlug);
      }
    })();

    inflightBySlug.set(tenantSlug, run);
    return run;
  },
