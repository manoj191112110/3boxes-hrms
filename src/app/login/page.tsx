...
import { isClientLiveMode, isClientDemoMode, DEMO_TENANT_SLUG, isVercelDemoHostname } from '@/lib/site-mode';
...
export default function LoginPage() {
  ...
  // Detect tenant from ?tenant= query or subdomain (e.g., marqaitechgroup.3boxeshrms.com)
  useEffect(() => {
    const detectTenant = async () => {
      try {
        ...
        if (!slug && (isClientDemoMode() || isVercelDemoHostname(hostname))) {
        ...
        if (slug) {
          const res = await fetch(`/api/public/tenant-info?slug=${encodeURIComponent(slug)}`);
          if (res.ok) {
  ...
  useEffect(() => {
    setIsDemoSite(isClientDemoMode());
    setIsPlatformSite(
      window.location.hostname === '3boxeshrms.com' ||
      window.location.hostname === 'www.3boxeshrms.com'
    );
  }, []);
          ...
          <div className="max-w-lg">
            ...
            {isDemoSite && (
              <div className="flex flex-wrap items-center gap-3 mb-8">
                <Link
                  href="/landing"
                  className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-white/10 border border-white/15 text-white text-sm font-semibold hover:bg-white/15 transition-all"
                >
                ...
                <Link
                  href="/register"
                  className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-green-500 to-emerald-500 text-white text-sm font-semibold shadow-lg shadow-green-500/25 hover:scale-[1.02] transition-all"
                >
      ...
      <div className="w-full lg:w-[45%] xl:w-[42%] flex items-center justify-center bg-white relative">
        ...
        <div className="relative z-10 w-full max-w-md px-8 py-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Free Trial Registration Link — demo site + generic login (not platform/tenant branded) */}
            {!isPlatformSite && (isDemoSite || !tenantInfo) && (
            <Link
              href="/register"
