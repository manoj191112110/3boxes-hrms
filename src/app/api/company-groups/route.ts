  ...
  await safe('ensureDefaultGroupsForTenants', async () => {
    ...
    for (const t of tenants) {
      try {
        ...
        // If we can't check, assume group is needed
        console.error(`[company-groups] Check for existing groups in tenant ${t.id} failed:`, e);
        tenantsNeedingGroup.push(t);
      }
    }
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // FILTER OUT hidden tenants (Marq AI Tech Pvt Ltd placeholder, demo tenant)
    // so they don't appear in the group companies list.
    const enriched = groups
      ...
      .map((g) => {
      const companiesWithCounts = g.companies.map((c) => ({
        ...c,
        _count: {
          ...(c._count || {}),
