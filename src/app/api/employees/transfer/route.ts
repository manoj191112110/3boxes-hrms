...
export async function GET(request: Request) {
  try {
    ...
    // No database model for transfer yet — return empty list
    return NextResponse.json(
      { transfers: [], pagination: { page: 1, limit: 20, total: 0, totalPages: 0 } },
      { headers: corsHeaders() }
    );
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const { employeeId, fromDepartmentId, toDepartmentId, transferDate, reason } = body;
    ...
    // No database model for transfer yet — return success
    return NextResponse.json(
      {
        success: true,
