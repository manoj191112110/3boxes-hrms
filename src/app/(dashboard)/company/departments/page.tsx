...
import { FiBriefcase, FiPlus, FiSearch, FiEye, FiEdit2, FiTrash2, FiX, FiInfo, FiAlertTriangle, FiSettings } from 'react-icons/fi'
import toast from 'react-hot-toast'
import { useCompanyData, apiFetch, getStatusBadge } from '@/components/company/useCompanyData'
import { DepartmentRecord } from '@/components/company/useCompanyData'
import { onlyAlphanumeric } from '@/lib/validators'
...
export default function DepartmentsPage() {
  const { companies, departments, loading, isAdmin, fetchDepartments, selectedCompanyId } = useCompanyData()

  const [search, setSearch] = useState('')
  const [showForm, setShowForm] = useState(false)
  const [editingId, setEditingId] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string, string>>({})
  const [viewingRecord, setViewingRecord] = useState<DepartmentRecord | null>(null)
  ...
  const fDepartments = departments.filter(d =>
    d.name.toLowerCase().includes(q) || (d.code || '').toLowerCase().includes(q)
  )

  const getDefaultForm = (): Record<string, string> => ({
    name: '', code: '', companyId: companies[0]?.id || '', status: 'active'
  })

  const getFormFromRecord = (r: DepartmentRecord): Record<string, string> => {
  ...
  const openEdit = (r: DepartmentRecord) => { setEditingId(r.id); setForm(getFormFromRecord(r)); setShowForm(true); setViewingRecord(null); setDeleteConfirmId(null) }
  const openView = (r: DepartmentRecord) => { setViewingRecord(r); setShowForm(false); setDeleteConfirmId(null) }
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (form.companyId) {
      const duplicateName = departments.find(d => d.name.toLowerCase() === form.name.toLowerCase() && d.companyId === form.companyId && d.id !== editingId)
      if (duplicateName) { toast.error('A department with this name already exists for this company'); return }
      if (form.code) {
        const duplicateCode = departments.find(d => (d.code || '').toLowerCase() === form.code.toLowerCase() && d.companyId === form.companyId && d.id !== editingId)
        if (duplicateCode) { toast.error('A department with this code already exists for this company'); return }
      }
    }
    ...
    try {
      ...
      await apiFetch(`/api/departments${cidParam}`, { method: isEdit ? 'PUT' : 'POST', body: JSON.stringify(body) })
  ...
  const handleDelete = async () => {
    ...
    try {
      const cidParam = selectedCompanyId ? `&companyId=${selectedCompanyId}` : ''
      await apiFetch(`/api/departments?id=${deleteConfirmId}${cidParam}`, { method: 'DELETE' })
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-thb-text-primary">Departments</h1>
            <p className="text-sm text-thb-text-secondary">Manage departments within your organization</p>
          </div>
        </div>
        {isAdmin && (
      ...
      <div className="relative">
        ...
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, code..." className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-thb-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400" />
      </div>
        ...
        <div className="thb-card border-l-4 border-l-teal-500">
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-thb-text-primary flex items-center gap-2"><FiEye className="w-5 h-5 text-teal-500" /> Department Details</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                ['Company', viewingRecord.company?.name], ['Employees', viewingRecord._count?.employees],
                ['Designations', viewingRecord._count?.designations],
              ].map(([label, value]) => (
      ...
      {showForm && (
        <div className="thb-card border-l-4 border-l-teal-500">
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-thb-text-primary">{editingId ? 'Edit' : 'Add'} Department</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div><label className="block text-xs font-medium text-thb-text-secondary mb-1">Name <span className="text-red-500 font-bold">*</span></label><input type="text" value={form.name || ''} onChange={e => updateForm('name', e.target.value)} required className={inputClass} placeholder="Department name" /></div>
                <div><label className="block text-xs font-medium text-thb-text-secondary mb-1">Code</label><input type="text" value={form.code || ''} onChange={e => updateForm('code', onlyAlphanumeric(e.target.value))} className={inputClass} placeholder="e.g. ENG" /></div>
                <div><label className="block text-xs font-medium text-thb-text-secondary mb-1">Company <span className="text-red-500 font-bold">*</span></label><select value={form.companyId || ''} onChange={e => updateForm('companyId', e.target.value)} required className={inputClass}><option value="">Select Company</option>{companies.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                  ...
                  {deleteConfirmId === d.id ? (
                    <td colSpan={6} className="px-4 py-3">
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2"><FiAlertTriangle className="w-4 h-4 text-red-500" /><span className="text-sm text-red-600">Delete <b>{d.name}</b>?</span></div>
                      ...
                      <TD className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {isAdmin && <>
                            ...
                            <button onClick={() => { setDeleteConfirmId(d.id); setDeleteName(d.name) }} className="p-1.5 rounded-lg text-thb-text-muted hover:text-red-500 hover:bg-red-50 transition-colors" title="Delete"><FiTrash2 className="w-3.5 h-3.5" /></button>
                          </>}
                        </div>
                      </TD>
                    </>
