          ...
          <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div><label className="block text-xs font-medium text-thb-text-secondary mb-1">Department *</label><select value={form.departmentId || ''} onChange={e => updateForm('departmentId', e.target.value)} required className={inputClass}><option value="">Select Department</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
