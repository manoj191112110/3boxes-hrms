...
function PreboardingContent() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/onboarding?limit=100', { headers: getAuthHeaders() });
      if (res.ok) {
        const data = await res.json();
        if (data.tasks && data.tasks.length > 0) {
          // API has data - would map to preboarding model
          // For now, keep demo data
        }
      }
    } catch {
        ...
        <div id="crud-form" className="thb-card border-l-4 border-l-emerald-500 animate-slide-in-down">
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-thb-text-secondary mb-1">Full Name *</label>
