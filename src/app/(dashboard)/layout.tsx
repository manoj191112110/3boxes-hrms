...
export default function DashboardLayout({
  ...
  // Auto-fix tenant branding once per browser session — never run schema
  // push here. POST /api/db-push applies hundreds of SQL statements and
  // blocks Dashboard / Home / Modules while it runs.
  useEffect(() => {
    if (!isAuthenticated) return;
    try {
      if (sessionStorage.getItem('tb_tenant_fixed')) return;
      sessionStorage.setItem('tb_tenant_fixed', '1');
    } catch { /* private mode */ }
    fetch('/api/fix-tenant', { method: 'POST' }).catch(() => {});
  }, [isAuthenticated]);

  // Fetch notifications — skip ticks while a request is still in flight
  useEffect(() => {
    if (isAuthenticated) {
      const initialTimeout = setTimeout(() => fetchNotifications(), 15000);
