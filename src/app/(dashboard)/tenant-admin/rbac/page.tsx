...
interface Company {
  ...
  name: string;
  code: string | null;
  companyGroup: { id: string; name: string; tenantId: string };
...
const roleTypeBadge = (role: Role) => {
  if (role.isSystem) return 'thb-badge thb-badge-warning';
  if (role.companyId) return 'thb-badge thb-badge-info';
  return 'thb-badge thb-badge-purple';
};

const roleTypeLabel = (role: Role) => {
...
// ─── Main Component ────────────────────────────────────────
export default function TenantAdminRBACPage() {
  ...
  const [roleForm, setRoleForm] = useState({
    name: '', key: '', description: '', level: 5, companyId: '',
  });
  ...
  const fetchRoles = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        let roleList: Role[] = data.roles || [];
        // Exclude super_admin roles for tenant admin
        roleList = roleList.filter((r) => r.key !== 'super_admin');
        setRoles(roleList);
        if (data.needsSeed) {
  ...
  }, [tenantId, roleSearch, selectedCompanyId]);
  ...
  }, [tenantId, userSearch, userCompanyFilter]);

  useEffect(() => { queueMicrotask(() => fetchCompanies()); }, [fetchCompanies]);
  useEffect(() => { if (activeTab === 'roles') queueMicrotask(() => fetchRoles()); }, [activeTab, fetchRoles]);
  useEffect(() => { if (activeTab === 'permissions') queueMicrotask(() => { fetchRoles(); fetchModules(); }); }, [activeTab, fetchRoles, fetchModules]);
  useEffect(() => { if (activeTab === 'users') queueMicrotask(() => fetchUsers()); }, [activeTab, fetchUsers]);
  ...
  const handleEditRole = (role: Role) => {
    ...
    setRoleForm({
      ...
      level: role.level, companyId: role.companyId || '',
    });
  };
  ...
  const handleCancelForm = () => {
    setShowRoleForm(false);
    setEditingRoleId(null);
    setRoleForm({ name: '', key: '', description: '', level: 5, companyId: '' });
  };
    ...
    try {
      ...
      const body = editingRoleId
        ...
        : {
            ...
            level: roleForm.level, tenantId, companyId: roleForm.companyId || null,
            isSystem: false, createdBy: user?.id,
          };
  ...
  const handleTogglePermission = (permissionId: string) => {
    setPermissionOverrides((prev) => ({ ...prev, [permissionId]: !prev[permissionId] }));
  };
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      const permissions = Object.entries(permissionOverrides).map(([permissionId, granted]) => ({ permissionId, granted }));
  ...
  const handleAssignRole = async () => {
    ...
    try {
      const res = await fetch(`/api/rbac/roles/${assignRoleId}/assign`, {
        ...
        body: JSON.stringify({ userId: assigningUserId, companyId: assignCompanyId || null, assignedBy: user?.id }),
      });
      if (!res.ok) { const d = await res.json(); throw new Error(d.error || 'Failed'); }
      toast.success('Role assigned');
      setAssigningUserId(null); setAssignRoleId(''); setAssignCompanyId('');
      fetchUsers();
  ...
  // ─── Input helpers ──────────────────────────────────────
  const inputCls = 'w-full px-3 py-2.5 rounded-lg border border-thb-border text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400';
  const labelCls = 'block text-xs font-medium text-thb-text-secondary mb-1';
  const selectCls = 'w-full px-3 py-2.5 rounded-lg border border-thb-border text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400 bg-white';
  const btnPrimary = 'inline-flex items-center gap-2 px-4 py-2.5 bg-teal-500 text-white hover:bg-teal-600 shadow-sm shadow-teal-500/25 rounded-lg font-medium text-sm transition-colors disabled:opacity-50';
  ...
  if (user?.role !== 'tenant_admin' && user?.role !== 'super_admin') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-thb-text-secondary">This page is only accessible to Tenant Admins and Super Admins.</p>
      </div>
    );
  }

  // Filter modules and categories based on user role — hide Admin-category modules for tenant_admin
  const isTenantAdmin = user?.role === 'tenant_admin';
  const filteredModules = isTenantAdmin ? modules.filter((m) => m.category !== 'Admin') : modules;
  const categories = [...new Set(filteredModules.map((m) => m.category))];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-thb-text-primary flex items-center gap-2">
            <FiShield className="w-6 h-6 text-teal-500" />
            Role & Permission Management
          </h1>
          <p className="text-thb-text-secondary mt-1">Manage company roles, permissions, and user access within your tenant</p>
        </div>
      ...
      <div className="thb-card p-4">
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-thb-text-primary">Company Scope:</span>
          </div>
          <select value={selectedCompanyId} onChange={(e) => setSelectedCompanyId(e.target.value)} className={selectCls + ' sm:w-64'}>
            <option value="">All Companies (Tenant-wide)</option>
      ...
      <div className="flex gap-1 bg-slate-100 p-1 rounded-lg w-fit overflow-x-auto">
        {[
          { key: 'roles' as const, label: 'Company Roles', icon: <FiLock className="w-4 h-4" /> },
          { key: 'permissions' as const, label: 'Permissions', icon: <FiGrid className="w-4 h-4" /> },
          { key: 'users' as const, label: 'User Access', icon: <FiUserCheck className="w-4 h-4" /> },
        ].map((tab) => (
      ...
      {activeTab === 'roles' && (
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" value={roleSearch} onChange={(e) => setRoleSearch(e.target.value)}
                placeholder="Search roles..." className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-thb-border text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20" />
            </div>
            {!showRoleForm && (
              <button onClick={() => { setShowRoleForm(true); setEditingRoleId(null); setRoleForm({ name: '', key: '', description: '', level: 5, companyId: selectedCompanyId }); }} className={btnPrimary}>
            ...
            <div ref={formRef} className="thb-card border-l-4 border-l-emerald-500 p-6 animate-slide-in-down">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleEditRole(viewingRole)} className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-teal-600 bg-teal-50 rounded-lg hover:bg-teal-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                ...
                <div><p className="text-xs text-thb-text-muted">Type</p><span className={roleTypeBadge(viewingRole)}>{roleTypeLabel(viewingRole)}</span></div>
              ...
              <div>
                ...
                <div className="max-h-48 overflow-y-auto space-y-1">
                  {(() => {
                    ...
                    return Object.entries(permByModule).map(([mod, acts]) => (
                      <div key={mod} className="flex items-center gap-2 text-xs">
                        ...
                        <div className="flex gap-1 flex-wrap">
                          {acts.map((a) => <span key={a} className="thb-badge thb-badge-success text-[10px]">{a}</span>)}
                        </div>
                      </div>
                    ));
                  })()}
          ...
          {/* Create/Edit Form */}
          {showRoleForm && (
            <div ref={formRef} className="thb-card border-l-4 border-l-teal-500 p-6 animate-slide-in-down">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-thb-text-primary">{editingRoleId ? 'Edit Role' : 'Create Company Role'}</h3>
              ...
              <form onSubmit={handleSubmitRole} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div><label className={labelCls}>Name <span className="text-red-500 font-bold">*</span></label><input required value={roleForm.name} onChange={(e) => setRoleForm({ ...roleForm, name: e.target.value })} className={inputCls} placeholder="Role name" /></div>
                  <div><label className={labelCls}>Key <span className="text-red-500 font-bold">*</span></label><input required value={roleForm.key} onChange={(e) => setRoleForm({ ...roleForm, key: e.target.value })} className={inputCls} placeholder="role_key" disabled={!!editingRoleId} /></div>
                  <div><label className={labelCls}>Level</label>
                    <select value={roleForm.level} onChange={(e) => setRoleForm({ ...roleForm, level: parseInt(e.target.value) })} className={selectCls}>
                      {[1, 2, 3, 4, 5].map((l) => <option key={l} value={l}>{levelLabel[l] || `Level ${l}`}</option>)}
                    </select>
                  </div>
                  <div><label className={labelCls}>Company Scope</label>
                    <select value={roleForm.companyId} onChange={(e) => setRoleForm({ ...roleForm, companyId: e.target.value })} className={selectCls}>
                      <option value="">All Companies (Tenant-wide)</option>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-thb-border bg-slate-50/50">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-thb-text-secondary uppercase tracking-wider">Company</th>
                      ...
                      <tr key={role.id} className="border-b border-thb-border/50 hover:bg-slate-50/50 transition-colors">
                        {deleteConfirmId === role.id ? (
                          ...
                          <>
                            ...
                            <td className="px-4 py-3"><span className={roleTypeBadge(role)}>{roleTypeLabel(role)}</span></td>
                            <td className="px-4 py-3 text-sm text-thb-text-secondary">{role.company?.name || <span className="flex items-center gap-1 text-thb-text-muted"><FiHome className="w-3 h-3" />All</span>}</td>
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center justify-end gap-1">
                                ...
                                {!role.isSystem && (
                                  <button onClick={() => handleEditRole(role)} className="p-2 rounded-lg text-thb-text-muted hover:text-teal-500 hover:bg-teal-50 transition-colors" title="Edit"><FiEdit2 className="w-4 h-4" /></button>
                                )}
                                {!role.isSystem && (
      ...
      {activeTab === 'permissions' && (
        <div className="space-y-4">
          ...
          <div className="thb-card p-4">
            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
              <div className="flex items-center gap-2">
                <FiShield className="w-4 h-4 text-teal-500" />
              ...
              <select value={selectedRoleId || ''} onChange={(e) => handleSelectRole(e.target.value)} className={selectCls + ' sm:w-64'}>
                <option value="">— Choose a role —</option>
                                {roles.filter((r) => !(isTenantAdmin && r.key === 'super_admin')).map((r) => (
                                  <option key={r.id} value={r.id}>
                                    {r.name} {r.company ? `(${r.company.name})` : ''} {r.isSystem ? '★' : ''}
                                  </option>
                                ))}
              </select>
              {selectedRoleId && selectedRole?.key !== 'super_admin' && (
          ...
          {selectedRole?.key === 'super_admin' && (
            <div className="thb-card p-4 bg-amber-50 border-amber-200">
              <p className="text-sm text-amber-700 flex items-center gap-2"><FiAlertTriangle className="w-4 h-4" /> Super Admin permissions cannot be modified from this view.</p>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" value={userSearch} onChange={(e) => setUserSearch(e.target.value)}
                placeholder="Search users..." className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-thb-border text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20" />
            </div>
                      ...
                      <tr key={u.id} className="border-b border-thb-border/50 hover:bg-slate-50/50 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-teal-100 text-teal-600 flex items-center justify-center text-xs font-bold flex-shrink-0">
                              {u.name?.[0] || '?'}
                            </div>
                            <div>
                        ...
                        <td className="px-4 py-3">
                          <span className={levelBadgeCls[
                            { tenant_admin: 1, hr_admin: 2, manager: 3, employee: 4 }[u.role] ?? 5
                              ...
                              <div className="flex items-center gap-2">
                                <select value={assignRoleId} onChange={(e) => setAssignRoleId(e.target.value)} className="text-xs px-2 py-1.5 rounded border border-thb-border bg-white">
                                  <option value="">Select role</option>
                                    {roles.filter((r) => !(isTenantAdmin && (r.key === 'super_admin' || r.key === 'tenant_admin'))).filter((r) => !r.isSystem || r.key === 'hr_admin' || r.key === 'finance_admin' || r.key === 'manager' || r.key === 'travel_admin' || r.key === 'crm_admin' || r.key === 'employee').map((r) => (
                                    <option key={r.id} value={r.id}>{r.name}{r.company ? ` (${r.company.name})` : ''}</option>
                                  ))}
                                </select>
                                <select value={assignCompanyId} onChange={(e) => setAssignCompanyId(e.target.value)} className="text-xs px-2 py-1.5 rounded border border-thb-border bg-white">
                                  <option value="">No company scope</option>
                                  {companies.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                                </select>
                                <button onClick={handleAssignRole} className="px-2 py-1.5 text-xs bg-teal-500 text-white rounded hover:bg-teal-600">Assign</button>
                                <button onClick={() => { setAssigningUserId(null); setAssignRoleId(''); setAssignCompanyId(''); }} className="px-2 py-1.5 text-xs border border-thb-border rounded hover:bg-slate-50">Cancel</button>
                              </div>
                            ) : (
                              <button onClick={() => { setAssigningUserId(u.id); setAssignCompanyId(selectedCompanyId); }} className="p-2 rounded-lg text-thb-text-muted hover:text-teal-500 hover:bg-teal-50 transition-colors" title="Assign Role">
...
function PermissionCategoryRow({
  ...
  const onColor = accentColor === 'teal' ? 'bg-teal-500' : 'bg-green-500';
              ...
              <td key={action} className="px-3 py-2.5 text-center">
                {perm ? (
                  <button
                    ...
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${isOn ? onColor : 'bg-slate-300'} ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                  >
