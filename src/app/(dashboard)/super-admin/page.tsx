...
export default function SuperAdminPage() {
  ...
  // ─── Register Tenant Databases (Golden Rule: separate DBs) ────────────
  const [registeringDbs, setRegisteringDbs] = useState(false);
  const handleRegisterTenantDbs = async () => {
    if (!confirm('Register separate databases for Demo and MarqAI tenants?\n\nThis ensures:\n• Super Admin (3boxeshrms.com) → neondb (platform DB)\n• Demo (nexus-hrms-mu.vercel.app) → tenant_demo\n• MarqAI (marqaitechgroup.3boxeshrms.com) → tenant_marqaitechgroup\n\nEach link will have its OWN separate database.')) return;
    try {
      setRegisteringDbs(true);
      const res = await fetch('/api/admin/register-tenant-dbs', {
        method: 'POST',
        headers: getAuthHeaders(),
      });
      if (!res.ok) {
      ...
      const data = await res.json();
      toast.success(data.message || 'Tenant databases registered successfully');
      console.log('Tenant DB registration:', data);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Failed to register tenant DBs');
    } finally {
      setRegisteringDbs(false);
    }
  };

  // ─── Seed Tenant Database ────────────────────────────
  const [seedingTenantDb, setSeedingTenantDb] = useState<string | null>(null);
  const handleSeedTenantDb = async (slug: string, name: string) => {
    if (!confirm(`Seed the dedicated database for "${name}"?\n\nThis will:\n• Register the TenantDatabase record\n• Create all required tables\n• Seed: Companies, Departments, Designations, Users, Employees, Leave Types, Shifts, Holidays\n• Password for all users: MarqAI@2026\n\nThis is safe to run multiple times (uses upsert/idempotent).`)) return;
    try {
      setSeedingTenantDb(slug);
      const res = await fetch(`/api/admin/seed-tenant-db?slug=${encodeURIComponent(slug)}`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
      if (!res.ok) {
      ...
      const data = await res.json();
      toast.success(data.message || `Tenant DB seeded successfully`, { duration: 8000 });
      console.log('Seed result:', data);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Failed to seed tenant DB');
    } finally {
      setSeedingTenantDb(null);
    }
  };

  // ─── Seed ALL Modules with sample data (DEMO ONLY) ───
  // Visible only on the demo site (nexus-hrms-mu.vercel.app).
  // Seeds 6 companies, 50 employees, and sample data for every module:
  // leave, attendance, payroll, recruitment, assets, training,
  // performance, grievances, helpdesk.
  const [seedingAllModules, setSeedingAllModules] = useState(false);
  const [seedAllWithReset, setSeedAllWithReset] = useState(false);
  const [seedPhaseLabel, setSeedPhaseLabel] = useState<string | null>(null);
  // Seed runs in 6 PHASES (one API call each) so no single serverless
  // request exceeds the Vercel timeout. Each phase is idempotent.
  const SEED_PHASE_LABELS: Record<number, string> = {
    1: 'Organisation structure',
    2: 'Employees',
    3: 'Leave',
    4: 'Attendance + Payroll',
    5: 'Recruitment + Assets',
    6: 'Training + Performance + Grievances + Helpdesk',
  };
  const handleSeedAllModules = async (withReset: boolean) => {
    if (withReset) {
      if (!confirm('RESET + SEED ALL MODULES (Demo only)?\n\n⚠️ This will FIRST DELETE all existing demo companies/employees/module data, then seed fresh:\n\n• 6 companies (TechNova, Innovatech, Global Dynamics, Apex Services, Prime Healthcare, Stellar)\n• 50 employees distributed across all 6 companies\n• Leave types + balances + 30 leave requests\n• Attendance: 30 days × 50 employees\n• Payroll: 3 months payslips × 50 employees + payroll runs\n• Recruitment: 6 job postings + 20 candidates + 30 applications\n• Assets: 30 assets + 25 assignments\n• Training: 5 trainings + 25 enrollments\n• Performance: 20 reviews + 40 goals\n• Grievances: 8 | Helpdesk tickets: 15\n\nThis cannot be undone. Continue?')) return;
    } else {
      if (!confirm('SEED ALL MODULES (Demo only)?\n\nThis will ADD sample data in 6 quick phases (idempotent — existing records are skipped):\n\n• 6 companies, 50 employees\n• Leave, attendance, payroll, recruitment, assets, training, performance, grievances, helpdesk sample data\n\nContinue?')) return;
    }
    try {
      setSeedingAllModules(true);
      setSeedAllWithReset(withReset);
      const totals: Record<string, { created: number; skipped: number }> = {};
      const failures: string[] = [];
      let lastMessage = '';
      for (let phase = 1; phase <= 6; phase++) {
        setSeedPhaseLabel(`Phase ${phase}/6 — ${SEED_PHASE_LABELS[phase]}…`);
        const url = `/api/admin/seed-demo-full?phase=${phase}${withReset && phase === 1 ? '&reset=true' : ''}`;
        const res = await fetch(url, { method: 'POST', headers: getAuthHeaders() });
        const d = await res.json().catch(() => ({} as any));
        if (!res.ok) {
          throw new Error(d.details ? `${d.error}: ${d.details}` : (d.error || `Phase ${phase} failed`));
        }
        lastMessage = d.message || lastMessage;
        const r = d.report || {};
        for (const [k, v] of Object.entries(r)) {
          if (k.startsWith('phase_')) continue;
          const rv = v as any;
          if (rv?.note && String(rv.note).startsWith('FAILED')) failures.push(`${k} — ${rv.note}`);
          if (!totals[k]) totals[k] = { created: 0, skipped: 0 };
          totals[k].created += rv?.created || 0;
          totals[k].skipped += rv?.skipped || 0;
        }
      }
      const lines = Object.entries(totals)
        .filter(([k]) => k !== 'reset')
        .map(([k, v]) => `${k}: +${v.created || 0}`)
        .join(' | ');
      if (failures.length) {
        toast.error(`Some phases reported errors:\n${failures.join('\n')}\n\n${lines}`, { duration: 15000 });
      } else {
        toast.success(`✅ Demo data seeded across all modules!\n\n${lines}`, { duration: 15000 });
      }
      console.log('[SeedAllModules] Report:', totals, lastMessage);
      fetchTenants();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Failed to seed demo data', { duration: 12000 });
    } finally {
      setSeedingAllModules(false);
      setSeedAllWithReset(false);
      setSeedPhaseLabel(null);
    }
  };

  // ─── Clear Probation Reviews ────────────────────────────
  const [clearingProbation, setClearingProbation] = useState<string | null>(null);
  const handleClearProbation = async (slug: string, name: string) => {
    if (!confirm(`Clear ALL probation review data for "${name}"?\n\nThis will delete all PerformanceReview records with reviewCycle='Probation'. This cannot be undone.`)) return;
    try {
      setClearingProbation(slug);
      const res = await fetch(`/api/admin/clear-probation?slug=${encodeURIComponent(slug)}`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
      if (!res.ok) {
      ...
      const data = await res.json();
      toast.success(data.message || 'Probation reviews cleared');
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Failed to clear probation reviews');
    } finally {
      setClearingProbation(null);
    }
  };

  // ─── Link Employee to User ────────────────────────────
  const [linkingEmployee, setLinkingEmployee] = useState<string | null>(null);
  const handleLinkEmployee = async (slug: string, name: string) => {
    if (!confirm(`Auto-link all users to their employee records for "${name}"?\n\nThis will find each user's employee record by email and set Employee.userId = User.id. This fixes the my-profile data issue.`)) return;
    if (!confirm(`Auto-link all users to their employee records for "${name}"?\n\nThis will:\n• Find each user's employee record by email\n• Set Employee.userId = User.id\n• Fix the my-profile data issue\n\nThis is safe and idempotent.`)) return;
    try {
      setLinkingEmployee(slug);
      ...
      const data = await res.json();
      toast.success(data.message || 'Employee records linked successfully');
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Failed to link employees');
    } finally {
  ...
  const fetchGroups = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Defense-in-depth: filter out groups belonging to hidden tenants + deduplicate by name+tenantId
        const seenGroupKeys = new Set<string>();
        setGroups(allGroups.filter((g: CompanyGroupRow) => {
          if (isTenantHiddenClient(g.tenantSlug || '') || (g.tenantName || '').includes(PLATFORM_PLACEHOLDER_NAME)) return false;
          // Deduplicate by name+tenantId (different IDs but same name = duplicate)
          const groupKey = `${g.tenantId || g.tenantSlug}:${g.name}`;
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'tenants' && (
        <div className="space-y-4">
            ...
            <div ref={formRef} className="thb-card border-l-4 border-l-green-500 p-6">
              ...
              <form onSubmit={handleSubmitTenant} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div><label className={labelCls}>Name <span className="text-red-500 font-bold">*</span></label><input required value={tenantForm.name} onChange={(e) => setTenantForm({ ...tenantForm, name: e.target.value })} className={inputCls} placeholder="Tenant name" /></div>
                  <div><label className={labelCls}>Slug <span className="text-red-500 font-bold">*</span></label><input required value={tenantForm.slug} onChange={(e) => setTenantForm({ ...tenantForm, slug: e.target.value })} className={inputCls} placeholder="tenant-slug" /></div>
                ...
                {!editingTenantId && (
                  <div className="pt-4 border-t border-thb-border">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div><label className={labelCls}>Admin Name <span className="text-red-500 font-bold">*</span></label><input required value={tenantForm.adminName} onChange={(e) => setTenantForm({ ...tenantForm, adminName: e.target.value })} className={inputCls} placeholder="Admin name" /></div>
                      <div><label className={labelCls}>Admin Email <span className="text-red-500 font-bold">*</span></label><input required type="email" value={tenantForm.adminEmail} onChange={(e) => setTenantForm({ ...tenantForm, adminEmail: e.target.value })} className={inputCls} placeholder="admin@company.com" /></div>
                      <div><label className={labelCls}>Admin Password <span className="text-red-500 font-bold">*</span></label><input required type="password" value={tenantForm.adminPassword} onChange={(e) => setTenantForm({ ...tenantForm, adminPassword: e.target.value })} className={inputCls} placeholder="••••••••" /></div>
                    </div>
                  </div>
                )}
          ...
          {/* ─── Demo-Only: Seed ALL Modules with sample data ───
               Visible ONLY on the demo site (nexus-hrms-mu.vercel.app).
               Hidden on the live site (3boxeshrms.com) per GOLDEN RULE. */}
          {!isClientLiveMode() && (
            <div className="thb-card p-4 bg-gradient-to-r from-fuchsia-50 to-purple-50 border-purple-200">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex-1">
                  <p className="text-sm font-bold text-purple-900 flex items-center gap-2">
                    <FiDatabase className="w-4 h-4" /> Demo Data Seeder
                  </p>
                  <p className="text-xs text-purple-700 mt-0.5">
                    Seeds the demo tenant with <b>6 companies</b>, <b>50 employees</b> (each company ≥ 6), and sample data for ALL modules:
                    leave, attendance, payroll, recruitment, assets, training, performance, grievances, helpdesk.
                  </p>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    onClick={() => handleSeedAllModules(false)}
                    disabled={seedingAllModules}
                    className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-white bg-gradient-to-r from-fuchsia-600 to-purple-600 rounded-lg hover:from-fuchsia-700 hover:to-purple-700 disabled:opacity-50 shadow-sm transition-all"
                    title="Add sample data (idempotent — existing records are skipped)"
                  >
                    {seedingAllModules && !seedAllWithReset ? <FiRefreshCw className="w-3.5 h-3.5 animate-spin" /> : <FiDatabase className="w-3.5 h-3.5" />}
                    {seedingAllModules && !seedAllWithReset && seedPhaseLabel ? seedPhaseLabel : 'Seed All Modules'}
                  </button>
                  <button
                    onClick={() => handleSeedAllModules(true)}
                    disabled={seedingAllModules}
                    className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-white bg-gradient-to-r from-red-600 to-rose-600 rounded-lg hover:from-red-700 hover:to-rose-700 disabled:opacity-50 shadow-sm transition-all"
                    title="DELETE existing demo data first, then seed fresh (destructive)"
                  >
                    {seedingAllModules && seedAllWithReset ? <FiRefreshCw className="w-3.5 h-3.5 animate-spin" /> : <FiTrash2 className="w-3.5 h-3.5" />}
                    {seedingAllModules && seedAllWithReset && seedPhaseLabel ? seedPhaseLabel : 'Reset + Seed Fresh'}
                  </button>
                </div>
              </div>
            </div>
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-100">
                        ...
                        <div className="ml-auto flex items-center gap-2 flex-wrap">
                          ...
                          <button onClick={() => handleSeedTenantDb(t.slug, t.name)} disabled={seedingTenantDb === t.slug} className="inline-flex items-center gap-1 px-2 py-1 text-[11px] font-medium text-indigo-700 bg-indigo-50 rounded-lg hover:bg-indigo-100 disabled:opacity-50" title="Seed dedicated tenant database with companies, employees, etc.">
                            {seedingTenantDb === t.slug ? <FiRefreshCw className="w-3 h-3 animate-spin" /> : <FiDatabase className="w-3 h-3" />}
                            Seed DB
                          </button>
                          <button onClick={() => handleClearProbation(t.slug, t.name)} disabled={clearingProbation === t.slug} className="inline-flex items-center gap-1 px-2 py-1 text-[11px] font-medium text-orange-700 bg-orange-50 rounded-lg hover:bg-orange-100 disabled:opacity-50" title="Clear ALL probation review data for this tenant">
                            {clearingProbation === t.slug ? <FiRefreshCw className="w-3 h-3 animate-spin" /> : <FiTrash2 className="w-3 h-3" />}
                            Clear Prob
                          </button>
                          <button onClick={() => handleLinkEmployee(t.slug, t.name)} disabled={linkingEmployee === t.slug} className="inline-flex items-center gap-1 px-2 py-1 text-[11px] font-medium text-cyan-700 bg-cyan-50 rounded-lg hover:bg-cyan-100 disabled:opacity-50" title="Auto-link all users to their employee records (fixes my-profile)">
                            {linkingEmployee === t.slug ? <FiRefreshCw className="w-3 h-3 animate-spin" /> : <FiUserCheck className="w-3 h-3" />}
                            Link Emp
                          </button>
          ...
          {showGroupForm && (
            <div className="thb-card border-l-4 border-l-teal-500 p-6">
              ...
              <form onSubmit={handleSubmitGroup} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className={labelCls}>Tenant (group company&apos;s parent) <span className="text-red-500 font-bold">*</span></label>
                    <select
                  ...
                  <div>
                    <label className={labelCls}>Group Company Name <span className="text-red-500 font-bold">*</span></label>
                    <input
            ...
            <div ref={formRef} className="thb-card border-l-4 border-l-green-500 p-6">
              ...
              <form onSubmit={handleSubmitPlan} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div><label className={labelCls}>Name <span className="text-red-500 font-bold">*</span></label><input required value={planForm.name} onChange={(e) => setPlanForm({ ...planForm, name: e.target.value })} className={inputCls} placeholder="Plan name" /></div>
                    <div><label className={labelCls}>Plan Type <span className="text-red-500 font-bold">*</span></label>
            ...
            <form onSubmit={handleSubmitApplyPlan} className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div>
                <label className={labelCls}>
                  <FiHome className="w-3 h-3 inline mr-1" />Group Company Parent (Tenant) <span className="text-red-500 font-bold">*</span></label>
                <select
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className={labelCls}>
                    <FiCalendar className="w-3 h-3 inline mr-1" />Start Date <span className="text-red-500 font-bold">*</span></label>
                  <input
                    type="date"
                ...
                <div>
                  <label className={labelCls}>
                    <FiCalendar className="w-3 h-3 inline mr-1" />End Date <span className="text-red-500 font-bold">*</span></label>
                  <input
                    type="date"
      ...
      {activeTab === 'backups' && (
        <div className="space-y-4">
          {/* Golden Rule: Register Separate Databases */}
          <div className="thb-card p-5 border-2 border-emerald-200 bg-emerald-50/30">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <h3 className="text-sm font-bold text-emerald-800 flex items-center gap-2">
                  <FiDatabase className="w-4 h-4" /> Golden Rule: Separate Databases
                </h3>
                <p className="text-xs text-emerald-700 mt-1">
                  Each link must have its OWN separate database. Click below to register the dedicated databases:
                </p>
                <ul className="text-xs text-emerald-600 mt-2 space-y-0.5 ml-4">
                  <li>• <strong>3boxeshrms.com</strong> (Super Admin) → neondb (platform DB)</li>
                  <li>• <strong>nexus-hrms-mu.vercel.app</strong> (Demo) → tenant_demo</li>
                  <li>• <strong>marqaitechgroup.3boxeshrms.com</strong> (Tenant) → tenant_marqaitechgroup</li>
                </ul>
              </div>
              <button
                onClick={handleRegisterTenantDbs}
                disabled={registeringDbs}
                className="inline-flex items-center gap-2 px-4 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 font-medium text-sm transition-colors disabled:opacity-50 whitespace-nowrap"
              >
                {registeringDbs ? <FiRefreshCw className="w-4 h-4 animate-spin" /> : <FiDatabase className="w-4 h-4" />}
                {registeringDbs ? 'Registering...' : 'Register Separate DBs'}
              </button>
            </div>
          </div>
