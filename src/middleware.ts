...
const DEMO_SLUG = DEMO_TENANT_SLUG
...
function extractTenantSlug(request: NextRequest): string {
  ...
  // ?tenant= query parameter (explicit override on live tenant subdomains)
...
function isDemoHostname(hostname: string): boolean {
  return DEMO_DOMAINS.includes(hostname);
}
...
export function middleware(request: NextRequest) {
  ...
  const isDemoDomain = isDemoHostname(hostname);

  // 2a. Demo showcase site (nexus-hrms-mu.vercel.app): marketing landing + free trial.
  //     Root URL shows the landing page; /register is the real trial signup form.
  if (isDemoDomain && pathname === '/') {
    const landingUrl = request.nextUrl.clone();
    landingUrl.pathname = '/landing';
    return NextResponse.redirect(landingUrl);
  }

  // 2b. Tenant subdomain redirect: real tenant subdomains should not see the public
  //     marketing landing — send them to branded login. Demo domain is excluded.
  if (tenantSlug && !isDemoDomain && (pathname === '/' || pathname.startsWith('/landing'))) {
