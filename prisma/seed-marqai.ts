...
async function main() {
  ...
  const designations: Array<{ id: string; title: string; companyIndex: number }> = [];
  for (let i = 0; i < createdCompanies.length; i++) {
    for (const def of designationDefs) {
      const dept = departments.find((d) => d.companyIndex === i && d.name === def.deptName);
      if (!dept) continue;
      ...
      designations.push({ id: des.id, title: des.title, companyIndex: i });
    }
  }
  ...
  for (let i = 0; i < createdCompanies.length; i++) {
    ...
    const hrDes = designations.find((d) => d.companyIndex === i && d.title === 'HR Manager');
